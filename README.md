...
- **GoogleGenerativeAIEmbeddings** (`models/embedding-001`) - Google's own embedding model, turns each chunk into an embedding vector

- **ChromaDB** - stores the embeddings and does the similarity search to find relevant chunks for a given question

- **Google Gemini API** (gemini-2.0-flash, Free Tier) - the actual LLM that generates the answer, using the retrieved chunks as context
